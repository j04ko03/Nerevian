  ...
  <AdminLayout>
    ...
    <div class="peticiones-container">
      ...
      <div v-else class="peticiones-list">
        <div v-for="peticion in peticiones" :key="peticion.id" class="peticion-card">
          <div class="card-main">
            ...
            <div class="card-body">
              ...
              <div class="message-box">
                <svg class="info-icon msg-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path>
                </svg>
                <p>{{ peticion.missatge || 'Sin mensaje adicional.' }}</p>
              </div>
            </div>
          </div>

          <div class="card-actions">
            <button @click="confirmApprove(peticion)" class="btn-approve" :disabled="processing === peticion.id">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" class="btn-icon">
                <polyline points="20 6 9 17 4 12"></polyline>
              </svg>
              <span>Aprobar</span>
            </button>
            <button @click="confirmReject(peticion)" class="btn-reject" :disabled="processing === peticion.id">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" class="btn-icon">
    ...
    <div v-if="showModal" class="modal-overlay">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          ...
          <button @click="rejectPeticion" class="btn-danger" :disabled="processing === selectedPeticion?.id">
            Confirmar Rechazo
          </button>
        </div>
      </div>
    ...
    <div v-if="showApproveModal" class="modal-overlay">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <button @click="showApproveModal = false" class="btn-secondary">Cancelar</button>
          <button @click="approvePeticion" class="btn-approve-modal" :disabled="processing === selectedPeticion?.id">
...
async function fetchPeticiones() {
  ...
  try {
    ...
    alert(err.response?.status === 403 ? 'No tienes permisos para ver esto.' : 'Error al cargar las peticiones.')
  } finally {
    loading.value = false
  }
}
...
async function approvePeticion() {
  ...
  try {
    await api.post(`/admin/registration-requests/${selectedPeticion.value.id}/approve`)
    peticiones.value = peticiones.value.filter(p => p.id !== selectedPeticion.value.id)
...
async function rejectPeticion() {
  ...
  processing.value = selectedPeticion.value.id
  try {
    await api.post(`/admin/registration-requests/${selectedPeticion.value.id}/reject`)
    peticiones.value = peticiones.value.filter(p => p.id !== selectedPeticion.value.id)
